...
class Room_Booking:
    def __init__ (self,root):
        ...
        # creating combo box ############
        combo_search_by=ttk.Combobox(table_frame,font=("arial",12,"bold"),width=20,state="readonly")
        combo_search_by["value"]=("Contact","Room")
        combo_search_by.current(0)
        combo_search_by.grid(row=0,column=2,padx=2)
        
        # creating entry field for searching the customer#####
        self.txt_search=StringVar()
        txt_search=ttk.Entry(table_frame,width=21,font=("arial",12,"bold"))
    ...
    def Fetch_Contact(self):
        if self.var_contact.get=="":
            ...
            if row==None:
                messagebox.showerror("ERROR","ENTERED NUMBER IS NOT MATCHED WITH RECORDS",parent=self.root)
            else:
                conn.commit()
                conn.close()
    ...
    def search(self):
        ...
        #if search_by == "" or search_txt=="":
        #    messagebox.showerror("Error", "Please select a search option and enter text")
        #    return
        #else:
        my_cursor.execute(
        "select * from rooms where " +
    ...
    def search(self):
        ...
        # 2. Allow only valid column names (VERY IMPORTANT)
        allowed_columns = {
            "Contact": "contact",
            "Room Type": "RoomType",
        ...
        column = allowed_columns[search_by]

        # 3. Safe parameterized query
        query = f"SELECT * FROM rooms WHERE {column} LIKE %s"
        value = ("%" + search_text + "%",)
